@extends('backend.app')
@section('title', 'Create Landing Page (Design 8 — Cool Conversion)')
@section('content')
...
<div class="content-wrapper">
    <div class="container-fluid py-3">
        <form action="{{ route('admin.landing_pages_nine.store') }}" method="POST" enctype="multipart/form-data" id="custom_landing_form">
            @csrf
                        ...
                        <div class="card-body p-4">
                            ...
                            <div id="packageTable">
                                <div class="pkg-edit-card mb-3 p-3" style="background:#f8fafc; border:1px solid #e2e8f0; border-radius:10px; position:relative;">
                                    ...
                                    <div class="mb-2"><label class="form-label small fw-bold mb-1">প্যাকেজ মূল্য (৳) *</label><input type="number" name="pkg_price[]" class="form-control form-control-sm" placeholder="যেমন: 1290" required></div>
                                    <div><label class="form-label small fw-bold mb-1">ডিসকাউন্ট টেক্সট</label><input type="text" name="pkg_discount_text[]" class="form-control form-control-sm" placeholder="যেমন: ৳190 সাশ্রয়"></div>
                                </div>
                            </div>
...
<script>
    $(document).ready(function() {
        ...
        // Add Package Row — Vertical Card Layout (event delegation)
        let pkgIndex = 1;
        $(document).on('click', '#addPkgRow', function(e) {
            e.preventDefault();
            let card = `<div class="pkg-edit-card mb-3 p-3" style="background:#f8fafc; border:1px solid #e2e8f0; border-radius:10px; position:relative;">
                <button type="button" class="btn btn-sm btn-danger remove-row" style="position:absolute; top:8px; right:8px; padding:4px 8px;"><i class="fas fa-times"></i></button>
        ...
        // Submit Form
        $('#custom_landing_form').submit(function(e) {
            e.preventDefault();
            e.stopImmediatePropagation(); // Prevents multiple global submissions
            ...
            $.ajax({
                ...
                error: function(xhr) {
                    toastr.error('Error occurred! Please check your inputs.');
