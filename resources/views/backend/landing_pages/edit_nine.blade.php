@extends('backend.app')
@section('title', 'Edit Landing Page (Design 8 — Cool Conversion)')
@section('content')
...
<div class="content-wrapper">
    <div class="container-fluid py-3">
        <form action="{{ route('admin.landing_pages_nine.update', $item->id) }}" method="POST" enctype="multipart/form-data" id="ajax_form">
            @csrf
            @method('PATCH')
            <input type="hidden" name="page_type" value="9">
            ...
            <div class="row">
                ...
                <div class="col-lg-4">
                    ...
                    <div class="premium-card" style="border:2px solid #2563eb;">
                        <div class="card-header-custom" style="background:#eff6ff;"><i class="fas fa-boxes"></i> প্যাকেজ ম্যানেজ</div>
                        <div class="card-body p-4">
                            <p class="small text-muted mb-3"><i class="fas fa-info-circle"></i> প্রতিটি প্যাকেজে — পরিমাণ (Qty), মূল্য ও ডিসকাউন্ট টেক্সট দিন</p>
                            <div id="packageTable">
                                @if($item->packages && $item->packages->count() > 0)
                                    ...
                                    <div class="pkg-edit-card mb-3 p-3" style="background:#f8fafc; border:1px solid #e2e8f0; border-radius:10px; position:relative;">
                                        ...
                                        <div class="mb-2"><label class="form-label small fw-bold mb-1">পরিমাণ (Qty) *</label><input type="number" name="pkg_qty[]" class="form-control form-control-sm" value="{{ $pkg->qty }}" required placeholder="যেমন: 2"></div>
                                        <div class="mb-2"><label class="form-label small fw-bold mb-1">প্যাকেজ মূল্য (৳) *</label><input type="number" name="pkg_price[]" class="form-control form-control-sm" value="{{ $pkg->price }}" required placeholder="যেমন: 2390"></div>
                                ...
                                @else
                                    <div id="no_pkg_row" class="text-center text-muted py-3" style="background:#f8fafc; border:1px dashed #cbd5e1; border-radius:8px;">
                                        ...
                                        <small>কোন প্যাকেজ নেই। নিচের "+ Add Package" বাটনে ক্লিক করুন</small>
                                    </div>
                                @endif
                            </div>
                            <button type="button" id="addPkgRow" class="btn btn-sm btn-success w-100"><i class="fas fa-plus"></i> + প্যাকেজ যোগ করুন</button>
                        </div>
                    </div>
    ...
    $(document).ready(function() {
        ...
        $(document).on('click', '#addPkgRow', function(e) {
            ...
            let tr = `<div class="pkg-edit-card mb-3 p-3" style="background:#f8fafc; border:1px solid #e2e8f0; border-radius:10px; position:relative;">
                ...
                <div class="mb-2"><label class="form-label small fw-bold mb-1">পরিমাণ (Qty) *</label><input type="number" name="pkg_qty[]" class="form-control form-control-sm" required placeholder="যেমন: 2"></div>
                <div class="mb-2"><label class="form-label small fw-bold mb-1">প্যাকেজ মূল্য (৳) *</label><input type="number" name="pkg_price[]" class="form-control form-control-sm" required placeholder="যেমন: 2390"></div>
        ...
        $('#ajax_form').submit(function(e) {
            ...
            $.ajax({
                ...
                success: function(res) {
                    window.location.href = res.url || "{{ route('admin.landing_pages_nine') }}";
                },
                error: function(xhr) {
