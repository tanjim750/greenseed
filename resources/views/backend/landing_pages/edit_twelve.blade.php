@extends('backend.app')
@section('title', 'Edit Landing Page (Design 11 — Honey)')
@section('content')
...
<div class="content-wrapper">
    <div class="container-fluid py-3">
        <form action="{{ route('admin.landing_pages_twelve.update', $item->id) }}" method="POST" enctype="multipart/form-data" id="ajax_form">
            @csrf
            @method('PATCH')
            <input type="hidden" name="page_type" value="12">
            ...
            <div class="row">
                <div class="col-lg-8">
                    ...
                    {{-- ৩.৫. বিস্তারিত বৈশিষ্ট্য (অর্গানিক/স্পেশাল ফিচার) - NEW SECTION ADDED HERE --}}
                    <div class="premium-card">
                        <div class="card-header-custom"><i class="fas fa-leaf"></i> ৩.৫. বিস্তারিত বৈশিষ্ট্য (অর্গানিক/স্পেশাল ফিচার)</div>
                        <div class="card-body p-4">
                            <div class="mb-3">
                                <label class="fw-bold">সেকশন টাইটেল</label>
                                <input type="text" name="special_feature_title" class="form-control" value="{{ $item->special_feature_title ?? 'কেন আমাদের মধু সত্যিকারের অর্গানিক?' }}">
                            </div>
                            <div class="row">
                                {{-- বাম কলাম --}}
                                <div class="col-md-6 border-end">
                                    <h6 class="fw-bold text-primary mb-3">বাম কলামের ফিচারসমূহ</h6>
                                    @for($i=1; $i<=4; $i++)
                                    <div class="section-row">
                                        <div class="mb-2">
                                            <label class="small fw-bold">টাইটেল {{ $i }}</label>
                                            <input type="text" name="sf_{{ $i }}_title" class="form-control" value="{{ $item->{'sf_'.$i.'_title'} }}" placeholder="যেমন: প্রাকৃতিক চাক থেকে সংগ্রহ">
                                        </div>
                                        <div>
                                            <label class="small fw-bold">বিস্তারিত {{ $i }}</label>
                                            <input type="text" name="sf_{{ $i }}_desc" class="form-control" value="{{ $item->{'sf_'.$i.'_desc'} }}" placeholder="সংক্ষিপ্ত বর্ণনা">
                                        </div>
                                    </div>
                                    @endfor
                                </div>
                                {{-- ডান কলাম --}}
                                <div class="col-md-6">
                                    <h6 class="fw-bold text-primary mb-3">ডান কলামের ফিচারসমূহ</h6>
                                    @for($i=5; $i<=8; $i++)
                                    <div class="section-row">
                                        <div class="mb-2">
                                            <label class="small fw-bold">টাইটেল {{ $i }}</label>
                                            <input type="text" name="sf_{{ $i }}_title" class="form-control" value="{{ $item->{'sf_'.$i.'_title'} }}" placeholder="যেমন: নিজস্ব তত্ত্বাবধানে প্রক্রিয়াজাত">
                                        </div>
                                        <div>
                                            <label class="small fw-bold">বিস্তারিত {{ $i }}</label>
                                            <input type="text" name="sf_{{ $i }}_desc" class="form-control" value="{{ $item->{'sf_'.$i.'_desc'} }}" placeholder="সংক্ষিপ্ত বর্ণনা">
        ...
        $('#ajax_form').submit(function(e) {
            ...
            $.ajax({
                ...
                success: function(res) {
                    window.location.href = res.url || "{{ route('admin.landing_pages_twelve') }}";
                },
                error: function(xhr) {
