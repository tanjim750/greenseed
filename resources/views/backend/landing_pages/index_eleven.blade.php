@extends('backend.app')
@section('title', 'Landing Pages (Type 11 — Design 10 — Skincare)')
...
<style>
    ...
    .custom-table { margin-bottom: 0; table-layout: fixed; width: 100%; }
    .custom-table th {
    ...
    .badge-soft-danger {
        ...
        font-weight: 600; font-size: 0.75rem; border: 1px solid #fecaca; display: inline-block;
    }
    ...
    .action-btn {
        ...
        border: none; margin: 0 3px; font-size: 15px; text-decoration: none !important; flex-shrink: 0;
    }
    ...
    @media (max-width: 767px) {
        ...
        .badge-soft-danger { font-size: 0.62rem; padding: 4px 8px; }
    ...
    @media (max-width: 575px) {
        ...
        .custom-table tbody tr {
            ...
            margin-bottom: 14px; padding: 12px; box-shadow: 0 4px 10px rgba(0,0,0,0.04);
        }
        ...
        .custom-table td:before {
            ...
            letter-spacing: 0.5px; text-align: left; flex-shrink: 0; width: 30%;
        }

        /* Flexbox wrapper to allow text truncation inside cards */
        .custom-table td .content-wrap {
            flex: 1; 
            min-width: 0; /* CRITICAL: allows flex children to shrink and truncate */
            display: flex;
                ...
                <div class="premium-card">
                    <div class="premium-header d-flex justify-content-between align-items-center flex-wrap gap-3">
                        <div class="d-flex align-items-center gap-3">
                            ...
                            <h4 class="premium-title">Landing Pages (Type 11 — Design 10)</h4>
                        </div>
                        <a href="{{ route('admin.landing_pages_eleven.create') }}" class="btn-create">
                    ...
                    <div class="card-body p-0">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td data-label="Headline">
                                            <div class="content-wrap" title="{{ $item->title1 }}">
                                                <div class="fw-bold fs-6 mb-1 text-dark truncate-text">
                                                    {{ $item->title1 ?: 'No Headline Provided' }}
                                                </div>
                                                <span class="badge-soft-danger truncate-text">
                                                    <i class="fas fa-layer-group me-1"></i> Skincare Layout 11
                                                </span>
                                            </div>
                                        </td>
                                        
                                        <td data-label="Product">
                                            <div class="content-wrap" title="{{ $item->product ? $item->product->name : 'Not Linked' }}">
                                                @if($item->product)
                                                    <div class="truncate-text">
                                                        <i class="fas fa-box text-muted me-1"></i> {{ $item->product->name }}
                                                    </div>
                                                @else
                                                    <span class="text-muted fst-italic truncate-text">Not Linked</span>
                                                @endif
                                            </div>
                                        </td>
                                        ...
                                        <td data-label="Live URL">
                                            <div class="content-wrap">
                                                <a href="{{ route('front.landing_pages_eleven.view_page', $item->id) }}" target="_blank" class="link-view">
                                        ...
                                        <td data-label="Actions" class="text-center">
                                            <div class="d-flex justify-content-center">
                                                <a href="{{ route('admin.landing_pages_eleven.edit', $item->id) }}" class="action-btn edit" data-bs-toggle="tooltip" title="Edit Content">
                                                    <i class="fas fa-pen"></i>
                                                </a>
                                                <a href="{{ route('admin.landing_pages.color', $item->id) }}" class="action-btn color" data-bs-toggle="tooltip" title="Settings">
                                                    <i class="fas fa-paint-roller"></i>
                                                </a>
                                                <form action="{{ route('admin.landing_pages_eleven.destroy', $item->id) }}" method="POST" class="d-inline delete-form">
                                                    @csrf
                                                    @method('DELETE')
                        ...
                        @if($items->hasPages())
                        <div class="p-4 border-top d-flex justify-content-center justify-content-md-end">
