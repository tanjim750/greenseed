...
<style>
    ...
    .table-premium thead th { background-color: #f0fdf4; color: #166534; font-weight: 600; text-transform: uppercase; font-size: 0.75rem; letter-spacing: 0.02em; border-bottom: 0; padding: 1rem; }
    ...
    .btn-action-edit { background-color: rgba(34, 197, 94, 0.1); color: #15803d; }
    .btn-action-edit:hover { background-color: #15803d; color: #fff; }
    ...
    .btn-create-green { background: linear-gradient(135deg, #15803d, #166534); color: #fff; border: none; padding: 10px 24px; border-radius: 50px; font-weight: 600; box-shadow: 0 4px 12px rgba(22, 101, 52, 0.25); }
    .btn-create-green:hover { color: #fff; transform: translateY(-1px); box-shadow: 0 6px 16px rgba(22, 101, 52, 0.35); }
</style>
@endpush
            ...
            <div class="card-body p-0">
                <div class="card-header-premium d-flex flex-column flex-md-row justify-content-between align-items-center gap-3">
                    <div class="d-flex align-items-center">
                        <h5 class="text-dark mb-0 fw-bold">Moringa Green Landing Pages</h5>
                        <span class="badge bg-success ms-2 rounded-pill">{{ count($items) }} Total</span>
                    </div>
                    <div class="d-flex gap-2">
                        <a href="{{ route('admin.landing_pages_fourteen.create') }}" class="btn btn-create-green">
                            <i class="mdi mdi-leaf me-1"></i> Create Moringa Page
                        </a>
                    </div>
                </div>
                        ...
                        <tbody>
                            @forelse ($items as $key => $item)
                                <tr>
                                    ...
                                    <td>
                                        <a href="{{ route('front.landing_pages_fourteen.view_page', ['id' => $item->id]) }}" target="_blank" class="btn-view-link">
                                            <i class="mdi mdi-open-in-new"></i> View Page
                                        </a>
                                    </td>
                                    <td class="text-end">
                                        <div class="d-flex justify-content-end gap-1">
                                            <a href="{{ route('admin.landing_pages_fourteen.edit', [$item->id]) }}" class="btn-action btn-action-edit" target="_blank" data-bs-toggle="tooltip" title="Edit Page">
                                                <i class="mdi mdi-pencil-outline"></i>
                                            </a>
                                            <button type="button" data-url="{{ route('admin.landing_pages_fourteen.destroy', [$item->id]) }}" class="btn-action btn-action-delete delete-page" data-bs-toggle="tooltip" title="Delete Page">
                                                <i class="mdi mdi-trash-can-outline"></i>
                                            </button>
                                            <form action="{{ route('admin.landing_pages_fourteen.destroy', [$item->id]) }}" method="POST" class="d-none delete-form">
                                                @csrf @method('DELETE')
                                            </form>
                                        </div>
                                    </td>
                                ...
                                <tr>
                                    <td colspan="4" class="text-center py-5">
                                        <div class="d-flex flex-column align-items-center justify-content-center">
                                            <i class="mdi mdi-leaf-maple text-success" style="font-size: 3rem; opacity: 0.5;"></i>
                                            <h5 class="text-muted mt-2">No Moringa pages found</h5>
                                            <p class="text-muted small">Click "Create Moringa Page" to add your first one</p>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
...
(function($){
    ...
    $(document).off('click.lp14_delete', '.delete-page').on('click.lp14_delete', '.delete-page', function(e){
