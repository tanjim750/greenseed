...
<style>
    ...
    .card-premium { border: none; box-shadow: 0 0.75rem 1.5rem rgba(18, 38, 63, 0.03); border-radius: 16px; background: #fff; overflow: hidden; }
    .card-header-premium { background-color: #fff; border-bottom: 1px solid #f1f3fa; padding: 1.5rem; }
    .table-premium thead th { background-color: #f8f9fa; color: #6c757d; font-weight: 600; text-transform: uppercase; font-size: 0.75rem; letter-spacing: 0.02em; border-bottom: 0; padding: 1rem; }
    ...
    .btn-action-edit { background-color: rgba(114, 124, 245, 0.1); color: #727cf5; }
    .btn-action-edit:hover { background-color: #727cf5; color: #fff; }
    ...
    .btn-view-link { display: inline-flex; align-items: center; gap: 5px; font-size: 0.85rem; font-weight: 600; color: #0acf97; background: rgba(10, 207, 151, 0.1); padding: 6px 12px; border-radius: 6px; transition: all 0.2s; text-decoration: none; }
    .btn-view-link:hover { background: #0acf97; color: #fff; }
</style>
@endpush
...
<div class="row">
    <div class="col-12">
        <div class="page-title-box d-sm-flex align-items-center justify-content-between py-3">
            <h4 class="page-title mb-0">Landing Page (Type 3 - Premium)</h4>
            <div class="page-title-right">
                <ol class="breadcrumb m-0">
                    ...
                    <li class="breadcrumb-item active">Type 3 Manage</li>
                </ol>
            </div>
        </div>
    </div>
            ...
            <div class="card-body p-0">
                <div class="card-header-premium d-flex flex-column flex-md-row justify-content-between align-items-center gap-3">
                    <div class="d-flex align-items-center">
                        <h5 class="text-dark mb-0 fw-bold">Premium Pages</h5>
                        <span class="badge bg-soft-primary text-primary ms-2 rounded-pill">{{ count($items) }} Total</span>
                    </div>
                    <div class="d-flex gap-2">
                        @can('product.create')
                            <a href="{{ route('admin.landing_pages_three.create') }}" class="btn btn-primary rounded-pill shadow-sm px-4">
                                <i class="mdi mdi-diamond-stone me-1"></i> Create Premium Page
                            </a>
                        @endcan
                    </div>
                </div>
                        ...
                        <tbody>
                            @forelse ($items as $key => $item)
                                <tr>
                                    ...
                                    <td>
                                        <a href="{{ route('front.landing_pages_three.view_page', ['id' => $item->id]) }}" target="_blank" class="btn-view-link">
                                    ...
                                    <td class="text-end">
                                        <div class="d-flex justify-content-end gap-1">
                                            @if(auth()->user()->can('permission.view'))
                                                <a href="{{ route('admin.landing_pages_three.edit', [$item->id]) }}" class="btn-action btn-action-edit" target="_blank" data-bs-toggle="tooltip" title="Edit Page">
                                                    <i class="mdi mdi-pencil-outline"></i>
                                                </a>
                                            @endif
                                            @if(auth()->user()->can('permission.view'))
                                                <button type="button" data-url="{{ route('admin.landing_pages_three.destroy', [$item->id]) }}" class="btn-action btn-action-delete delete-page" data-bs-toggle="tooltip" title="Delete Page">
                                                    <i class="mdi mdi-trash-can-outline"></i>
                                                </button>
                                                <form action="{{ route('admin.landing_pages_three.destroy', [$item->id]) }}" method="POST" class="d-none delete-form">
                                                    @csrf @method('DELETE')
                                                </form>
                                            @endif
                                        </div>
                                ...
                                <tr>
                                    <td colspan="4" class="text-center py-5">
                                        <div class="d-flex flex-column align-items-center justify-content-center">
                                            ...
                                            <h5 class="text-muted mt-2">No Type 3 pages found</h5>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    ...
                    error: function(xhr){ console.error(xhr); btn.data('deleting', 0); if(form.length){ form.submit(); } }
                });
            }
        });
    });
