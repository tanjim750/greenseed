  ...
  @forelse($orders as $item)
    ...
    <div class="invoice-card">
      ...
      <div class="row g-3 mb-3">
        ...
        <div class="col-lg-5">
          <div class="border rounded p-3 h-100">
            ...
            <div class="d-flex justify-content-between mb-2"><span class="bold">Created</span><span>{{ $item->created_at->diffForHumans() }}</span></div>
            
            <div class="border-top pt-2 mt-2">
                <div class="d-flex justify-content-between mb-1">
                    <span class="bold">Payment Method</span>
                    <span class="badge bg-light text-dark border">{{ $item->payment_method ?? 'Cash on Delivery' }}</span>
                </div>
      ...
      <div class="row mt-3 g-3 align-items-start">
        ...
        <div class="col-lg-6">
          @php
            $deliveryAmount = $item->shipping_charge ?? 0;
            
            $totalDiscount = $item->discount ?? 0;
            
            $productLevelDiscount = 0;
            foreach($item->details as $d) {
                $productLevelDiscount += ($d->discount ?? 0) * ($d->quantity ?? 0);
            }
            
            $couponDiscount = $totalDiscount - $productLevelDiscount;
            if($couponDiscount < 0) { $couponDiscount = 0; } 
            
            $grandTotal = $item->final_amount ?? (($subTotal + $deliveryAmount) - $totalDiscount);
          @endphp
          ...
          <div class="border rounded p-3 totals">
            ...
            <div class="d-flex justify-content-between text-danger">
              ...
              <h6>- {{ priceFormate($couponDiscount) }}</h6>
            </div>
            @endif
