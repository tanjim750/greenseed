...
<head>
    ...
    <style>
        ...
        .hero-badge { display: inline-flex; align-items: center; gap: 6px; background: var(--honey-100); color: var(--honey-800); padding: 6px 16px; border-radius: 50px; font-size: 0.82rem; font-weight: 700; margin-bottom: 18px; border: 1px solid var(--honey-300); }
        ...
        .product-details-sec { background: var(--cream); }
        ...
        .review-card { background: var(--cream); border-radius: 14px; padding: 22px 20px; height: 100%; border: 1px solid var(--honey-200); }
        ...
        footer .ftagline { color: var(--honey-700); font-size: 0.92rem; margin-bottom: 16px; font-style: italic; }
        ...
        @media (max-width: 768px) {
            ...
            .btn-primary-cta, .btn-secondary-cta { padding: 12px 22px; width: 100%; justify-content: center; }
            .sec { padding: 32px 0; }
            ...
            .mobile-sticky .btn-primary-cta { width: 100%; justify-content: center; padding: 12px; font-size: 0.95rem; }
            body { padding-bottom: 76px; }
        }
        @media (min-width: 769px) { .mobile-sticky { display: none; } }
/* ✅ Single variation hide (Page 5 pattern) */
...
<section class="hero">
    <div class="hero-content">
        @if(!empty($ln_pg->promise_img_badge))
            <div class="hero-badge">{{ $ln_pg->promise_img_badge }}</div>
        @endif
        
        <h1 class="hero-title">{{ $ln_pg->title1 ?? 'Honeyraj' }}</h1>
        
        @if(!empty($ln_pg->title2))
            <p class="hero-sub">{{ $ln_pg->title2 }}</p>
        @endif
        ...
        <div class="price-block mt-3">
            ...
            @if(!empty($ln_pg->discount_save_text))<span class="price-save">{{ $ln_pg->discount_save_text }}</span>@endif
        </div>
...
{{-- GALLERY SLIDER (Bootstrap Carousel) --}}
...
<section class="sec" style="background:#f8fafc; padding: 36px 0;">
    <div class="container">
        ...
        <div class="mx-auto" style="max-width: 650px; background: #fff; padding: 12px; border-radius: 16px; box-shadow: 0 8px 24px rgba(0,0,0,0.08);">
                ...
                <div class="carousel-inner" style="border-radius: 12px; overflow: hidden;">
                    @foreach($ln_pg->images as $key => $img)
                        <div class="carousel-item {{ $key == 0 ? 'active' : '' }}">
                            <img src="{{ asset('landing_sliders/'.$img->image) }}" class="d-block w-100" alt="Gallery {{ $key + 1 }}" style="max-height: 450px; object-fit: contain; background: #f8fafc;">
                        </div>
                    @endforeach
                </div>
                @if($ln_pg->images->count() > 1)
                <button class="carousel-control-prev" type="button" data-bs-target="#productGallerySlider" data-bs-slide="prev">
                    <span class="carousel-control-prev-icon" aria-hidden="true" style="background-color: rgba(0,0,0,0.5); border-radius: 50%; padding: 22px;"></span>
                </button>
                <button class="carousel-control-next" type="button" data-bs-target="#productGallerySlider" data-bs-slide="next">
                    <span class="carousel-control-next-icon" aria-hidden="true" style="background-color: rgba(0,0,0,0.5); border-radius: 50%; padding: 22px;"></span>
                </button>
                @endif
            </div>
        </div>
...
{{-- ৩.৫. বিস্তারিত বৈশিষ্ট্য (অর্গানিক/স্পেশাল ফিচার) - NEW DYNAMIC SECTION --}}
@if(!empty($ln_pg->special_feature_title) || !empty($ln_pg->sf_1_title) || !empty($ln_pg->left_side_desc))
<section class="sec authenticity">
    <div class="container">
        <div class="sec-title-wrap">
            @if(!empty($ln_pg->identify_badge))
                <span class="sec-pill">{{ $ln_pg->identify_badge }}</span>
            @endif
            <h2 class="sec-title">{{ $ln_pg->special_feature_title ?? 'বিস্তারিত জানুন' }}</h2>
            @if(!empty($ln_pg->identify_subtitle))
                <p class="sec-sub">{{ $ln_pg->identify_subtitle }}</p>
            @endif
        </div>
        <div class="auth-grid">
            <div class="auth-card">
                <div class="head"><div class="ic"><i class="fas fa-check-circle"></i></div><h3>{{ $ln_pg->left_side_title ?? 'বিস্তারিত জানুন' }}</h3></div>
                <div class="body">
                    @if(!empty($ln_pg->sf_1_title) || !empty($ln_pg->sf_2_title) || !empty($ln_pg->sf_3_title) || !empty($ln_pg->sf_4_title))
                        <ul>
                            @for($i=1; $i<=4; $i++)
                                @if(!empty($ln_pg->{'sf_'.$i.'_title'}))
                                    <li>
                                        <strong>{{ $ln_pg->{'sf_'.$i.'_title'} }}</strong>
                                        @if(!empty($ln_pg->{'sf_'.$i.'_desc'}))
                                            — {{ $ln_pg->{'sf_'.$i.'_desc'} }}
                                        @endif
                                    </li>
                                @endif
            ...
            <div class="auth-card">
                <div class="head"><div class="ic"><i class="fas fa-heart"></i></div><h3>{{ $ln_pg->right_side_title ?? 'বিস্তারিত জানুন' }}</h3></div>
                <div class="body">
                    @if(!empty($ln_pg->sf_5_title) || !empty($ln_pg->sf_6_title) || !empty($ln_pg->sf_7_title) || !empty($ln_pg->sf_8_title))
                        <ul>
                            @for($i=5; $i<=8; $i++)
                                @if(!empty($ln_pg->{'sf_'.$i.'_title'}))
                                    <li>
...
<section class="sec product-details-sec">
    <div class="container">
        ...
        <div class="pd-grid">
            <div class="pd-img"><img src="{{ $heroImage }}" alt="{{ $productName }}"></div>
            <div class="pd-info">
                <h3>{{ $productName }}</h3>
                <ul class="spec-list">
                    @for($i=1; $i<=7; $i++)
...
<section class="sec reviews-sec">
    <div class="container">
        <div class="sec-title-wrap">
            ...
            @if(!empty($ln_pg->review_subtitle))
    ...
    <div class="container">
        ...
        <div class="order-card">
            <form id="checkout_land_form" action="{{ route('front.storelandData') }}" method="POST">
                ...
                <button type="submit" id="submit_btn" class="btn-order-confirm">{{ $ln_pg->btn_text_form ?? 'অর্ডার কনফার্ম করুন' }} — ৳<span id="btn_total">{{ $defaultPrice }}</span></button>
...
<footer>
    <div class="container">
        ...
        @if(!empty($ln_pg->footer_copyright))
...
<div class="mobile-sticky">
    <a href="#order-form" class="btn-primary-cta"><i class="fas fa-shopping-cart"></i> {{ $ln_pg->btn_text_hero ?? 'অর্ডার করুন' }} — ৳<span id="btn_total_mob">{{ $defaultPrice }}</span></a>
</div>
...
<script>
    ...
    function recalc() {
        ...
        $('#btn_total_mob').text(grand); // Update mobile sticky bottom bar
    ...
    function applyCouponLand() {
        ...
        $.ajax({
            ...
            success: function(res) { if(res.success) { $('#hidden_coupon_code').val(code); $('#hidden_discount').val(res.discount); $('#coupon_msg').text(res.msg).css('color','green'); } else { $('#hidden_coupon_code').val(''); $('#hidden_discount').val(0); $('#coupon_msg').text(res.msg).css('color','red'); } recalc(); },
